...
void AudioManager::guiSetup() {
    ...
    //------------------------------------------------//
    gui.add(sampleHitGain.set("sample Hit Gain", 2.3, 0.0, 4.0));
    ...
    gui.add(polySynthGain.set("poly Synth Gain", 0.6, 0.0, 1.0));
    ...
    gui.add(squareBassGain.set("square Bass Gain", 0.12, 0.0, 1.0));
    
    gui.add(sawSynthGain.set("saw Synth Gain", 0.5 , 0.0, 1.0));
...
void AudioManager::update(const ofVec2f player_position) {
    ...
    if (player_pos.x > partWorldWidth) {
        playerSynthLineAttack = ofMap(player_pos.x, partWorldWidth, HALF_WORLD_WIDTH, 4, 500);
        playerSynthLineRelease = ofMap(player_pos.x, partWorldWidth, HALF_WORLD_WIDTH, 600, 800);
    } else if (player_pos.x < -partWorldWidth){
        playerSynthLineAttack = ofMap(player_pos.x, -partWorldWidth, -HALF_WORLD_WIDTH, 4, 500);
        playerSynthLineRelease = ofMap(player_pos.x, -partWorldWidth, -HALF_WORLD_WIDTH, 600, 200);
    } else {
        playerSynthLineAttack = 4;
